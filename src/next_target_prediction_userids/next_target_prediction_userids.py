...
class SwiGLU(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int):
        """Initialize the SwiGLU activation function."""
        super().__init__()
...
class NextTargetPredictionBatch:
    ...
    actor_history_actions: (
        torch.Tensor
    )  # [B, N] - list of last N social actions taken by the actor
    actor_history_targets: (
        torch.Tensor
    )  # [B, N] - list of last N UserIds these actions have been taken on
    actor_history_mask: (
        torch.Tensor
    )  # [B, N] - validity mask for variable-length histories
    example_action: (
        torch.Tensor
    )  # [B] - action taken on the target of this training example
...
class NextTargetPredictionUserIDs(nn.Module):
        ...
        if self.history_encoder_type == "transformer":
            self.history_encoder = nn.TransformerEncoder(
                nn.TransformerEncoderLayer(
                    ...
                    * 2,  # action_embedding + target_user_embedding
                    nhead=8,
            ...
            self.learnable_queries = nn.Parameter(
                torch.randn(self.num_attention_heads, embedding_dim * 2)
            )
            ...
            raise ValueError(
                f"Unknown history_encoder_type: {self.history_encoder_type}"
            )
    ...
    def _encode_history_with_transformer(
        ...
        action_embeds = self.action_embeddings(
            history_actions
        )  # [B, N, D_emb] - action embeddings
        target_embeds = self.user_embeddings(
            history_targets
        )  # [B, N, D_emb] - target user embeddings
        ...
        history_embeds = torch.cat(
            [action_embeds, target_embeds], dim=-1
        )  # [B, N, D_emb * 2]
        ...
        encoded_history = self.history_encoder(
            history_embeds, src_key_padding_mask=attention_mask
        )  # [B, N, D_emb * 2]
    ...
    def _encode_history_with_simple_attention(
        ...
        if attention_mask_padding.all():
            ...
            return torch.zeros(
                batch_size, seq_len, self.embedding_dim, device=self.device
            )
        ...
        raw_attention_scores = torch.einsum(
            "kd,bnd->kbn", self.learnable_queries, history_embeds
        )
        ...
        attention_scores = attention_scores.masked_fill(
            attention_mask_padding.unsqueeze(0).unsqueeze(2), float("-inf")
        )
        # attention_scores: [num_attention_heads, B, N, N]
        ...
        weighted_sum = weighted_sum.reshape(
            batch_size, seq_len, self.num_attention_heads * embed_dim_combined
        )
    ...
    def encode_history_for_target(
        ...
        if self.history_encoder_type == "simple_attention":
            encoded_history = self._encode_history_with_simple_attention(
                history_actions, history_targets, history_mask
            )  # [B, N, D_emb]
        else:
    ...
    def forward(
        ...
        history_repr = self.encode_history_for_target(
            actor_history_actions, actor_history_targets, actor_history_mask
        )  # [B, D_emb]
    ...
    def train_forward_with_target(
        self, batch: NextTargetPredictionBatch, num_rand_negs: int = 0
        ...
        target_mask = (
            batch.example_target.unsqueeze(1) == batch.example_target.unsqueeze(0)
        ).float()  # [B, B]
        ...
        pos_scores = torch.diag(
            logits
        )  # [B] - scores for positive examples (from in-batch part)
        ...
        return {
            ...
            "num_negatives": torch.tensor(
                all_logits.size(1) - 1, device=self.device
            ),  # Total number of negatives per example
        }

    def temporal_pretraining_loss(
        self, batch: NextTargetPredictionBatch, num_temporal_examples: int = 8
    ) -> Dict[str, torch.Tensor]:
        ...
        for i in range(num_temporal_examples):
            ...
            if (
                pos >= 0 and pos < history_length - 1
            ):  # Need at least one position after for prediction
        ...
        temporal_reprs = encoded_history[
            :, temporal_positions
        ]  # [B, num_temporal, D_emb]
        temporal_actions = batch.actor_history_actions[
            :, [p + 1 for p in temporal_positions]
        ]  # [B, num_temporal]
        temporal_targets = batch.actor_history_targets[
            :, [p + 1 for p in temporal_positions]
        ]  # [B, num_temporal]
        temporal_valid = batch.actor_history_mask[
            :, [p + 1 for p in temporal_positions]
        ]  # [B, num_temporal]
        ...
        temporal_valid = (
            temporal_valid.bool()
            & (temporal_targets < self.num_users)
            & (temporal_targets >= 0)
        )

        # Get actor embeddings (same for all temporal positions)
        actor_embeds = self.user_embeddings(batch.actor_id)  # [B, D_emb]
        actor_embeds_expanded = actor_embeds.unsqueeze(1).expand(
            -1, len(temporal_positions), -1
        )  # [B, num_temporal, D_emb]
        ...
        actor_history_interaction = (
            actor_embeds_expanded * temporal_reprs
        )  # [B, num_temporal, D_emb]
        actor_action_interaction = (
            actor_embeds_expanded * action_embeds
        )  # [B, num_temporal, D_emb]
        ...
        unified_repr = torch.cat(
            ...
            dim=-1,
        )  # [B, num_temporal, D_emb * 5]

        # Reshape to process all temporal positions together
        batch_size, num_temporal = unified_repr.shape[:2]
        unified_repr_flat = unified_repr.view(
            -1, self.embedding_dim * 5
        )  # [B * num_temporal, D_emb * 5]

        # Pass through interaction network
        actor_action_reprs_flat = self.interaction_network(
            unified_repr_flat
        )  # [B * num_temporal, D_emb]

        # Reshape back to [B, num_temporal, D_emb]
        actor_action_reprs = actor_action_reprs_flat.view(
            batch_size, num_temporal, self.embedding_dim
        )  # [B, num_temporal, D_emb]
        ...
        actor_action_reprs_flat = actor_action_reprs.view(
            -1, self.embedding_dim
        )  # [B * num_temporal, D_emb]
        logits = torch.matmul(
            actor_action_reprs_flat, all_target_embeds.t()
        )  # [B * num_temporal, num_users]

        # Reshape back to [B, num_temporal, num_users]
        logits = logits.view(
            batch_size, len(temporal_positions), self.num_users
        )  # [B, num_temporal, num_users]
        ...
        masked_logits_flat = masked_logits.view(
            -1, self.num_users
        )  # [B * num_temporal, num_users]
    ...
    def train_forward(
        ...
        # Combine losses
        combined_loss = (1 - temporal_weight) * standard_results[
            "loss"
    ...
    def predict_top_k(
        ...
        with torch.no_grad():
            scores = torch.sum(
                actor_action_repr.unsqueeze(1) * candidate_embeds, dim=-1
            )  # [B, C]

        # Get top-k predictions
        top_k_scores, top_k_indices = torch.topk(
            scores, k=min(k, candidate_targets.size(1)), dim=1
        )
